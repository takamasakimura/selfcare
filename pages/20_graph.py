...
st.header('Graph1｜就寝・起床のベースラインからのズレ（±5h）')
...
g1 = make_sleep_dev(recent)
if not g1.empty:
    mdf = g1.melt(id_vars=['日付','日付_str'], var_name='系列', value_name='偏差時間(h)')
...
with tab_w:
    ...
    num = (
        w_m   * comp['精神的要求（Mental Demand）'] +
        w_p   * comp['身体的要求（Physical Demand）'] +
        w_t   * comp['時間的要求（Temporal Demand）'] +
        w_e   * comp['努力度（Effort）'] +
        w_perf* comp['__perf__'] +
        w_f   * comp['フラストレーション（Frustration）']
    )
